...
full_times = math.ceil(size / speed)
part = 0

for times in range(1, full_times + 1):
    download = speed * times
    part += math.ceil((speed / size) * 100)
    if part >= 100 or download >= size:
        part = 100
        download = size
    print('Прошло ' + str(times) + ' сек. Скачано', str(download) + ' из ' + str(size) + ' Мб (' + str(part) + '%)')
    
print('Скачивание обновления займет', full_times, 'сек')